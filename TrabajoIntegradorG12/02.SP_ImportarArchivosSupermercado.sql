...
CREATE OR ALTER PROCEDURE Supervisor.InsertarEmpleadosEncriptado --solo el Supervisor por que el solo sabe la contrase�a
    @rutaArchivo NVARCHAR(MAX),
    ...
    BEGIN TRY
        ...
        DECLARE @sql NVARCHAR(MAX) = '
            ...
            FROM OPENROWSET(
                ''Microsoft.ACE.OLEDB.12.0'', 
                ''Excel 12.0 Xml;HDR=NO;Database=' + @rutaArchivo + ''', 
                ''SELECT * FROM [medios de pago$B3:C]''  -- Aqu� se ajusta el rango
            ) AS ExcelData';
